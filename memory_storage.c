...
uint32_t const config_flash_addr = 0x0003F000;
...
{
	uint32_t* pb = (uint32_t*)init_packet_flash_addr;
	uint8_t* p = (uint8_t*)pb;
	return p;
}
...
{
	uint32_t* pb = (uint32_t*)firmware_image_flash_addr;
	uint8_t* p = (uint8_t*)pb;
	return p;
}
...
void fstorage_clear_flash(flash_addr_type_t type, uint32_t* page_offset_p)
{
	...
	switch (type) {
		...
		case FLASH_CONFIG: {
			nrf_fstorage_erase(&fstorage, config_flash_addr, 1, NULL);
		} break;
	}
...
//			NRF_LOG_INFO("addr = %X8 \r\n", firmware_image_flash_addr + offset);
		...
		case FLASH_CONFIG: {
			rc = nrf_fstorage_write(&fstorage, config_flash_addr, &image_data_for_write, len, NULL);
		} break;
	}
	wait_for_flash_ready(&fstorage);
...
uint8_t fstorage_read_flash(flash_addr_type_t type, uint8_t* data_p, uint32_t* size_p, uint32_t* page_offset_p)
{
	...
	switch (type) {
		...
		case FLASH_CONFIG: {
			rc = nrf_fstorage_read(&fstorage, config_flash_addr, data_p, size);
		} break;
	}
	if (rc != NRF_SUCCESS)
		return 0;
